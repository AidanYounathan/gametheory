...
export default function Home() {
    ...
    return (
        <div className=" text-bone">
            <Marquee className="overflow-hidden font-bold text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl tracking-normal text-background text-outline">SELECT YOUR OPPONENT! SELECT YOUR OPPONENT! SELECT YOUR OPPONENT! SELECT YOUR OPPONENT!&nbsp;</Marquee>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2 sm:gap-3 md:gap-4 lg:gap-6 p-4 sm:p-6 md:p-8 md:py-3 max-w-7xl mx-auto">
                {/* Character options - 5x3 responsive grid */}
                {characters.map((character) => (
                    <div 
                        key={character.id}
                        onClick={() => handleCharacterSelect(character.id)}
                        className={`bg-gray-200 border p-2 sm:p-3 lg:p-4 rounded-lg h-32 sm:h-36 md:h-40 lg:h-48 flex flex-col justify-between items-center text-center cursor-pointer transition-all duration-200 ${
                    ...
                    >
                        <div className="w-full flex-1 flex items-center justify-center mb-2">
                            <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 bg-gray-300 rounded-full flex items-center justify-center">
                                <span className="text-gray-500 text-xs lg:text-sm">IMG</span>
                                {/* <Image 
                                    src={character.image} 
                                    alt={character.name}
                                    width={80}
                        ...
                        <div className="w-full">
                            <h3 className="text-xs sm:text-sm lg:text-sm font-bold text-gray-800 mb-0.5">
                                {character.name}
                            </h3>
                            <p className="text-[10px] sm:text-xs lg:text-xs text-gray-600 leading-tight">
